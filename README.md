...
| 同じ会員が時間帯の重なる別の枠を取らない | 確定時に、その会員の予約と時間の重なりを確かめる（`開始 < 相手の終了 かつ 終了 > 相手の開始`。境界が一致するだけの隣接は重複としません） |
...
満席のときはここでは断るところまでで、キャンセル待ちへの登録は STEP5、マイ予約は STEP6 です。

同時実行は「2 本のトランザクションを実際に張る」テストで固定しています（`ReservationConcurrencyTest`）。
先行が枠を押さえている間、後続は**行ロックの解放を待って進めない**こと（待たされている文が
`lesson_slots … for update` であること）と、先行が確定したあとの後続は**満席で断られる**ことを確かめます。
行ロックを外すとこのテストは落ちます。
...
| 講師・運営 | `staff@example.com` | 枠の開講・編集（自分＝佐倉 みなとの枠のみ）、予約状況の把握 |
...
ひととおり入っています。定期スケジュール（毎週 火・木の朝ヨガ、水の夜クラス など）は
繰り返し開講の仕組み（`RecurringSlots`）でそのまま生成しているので、画面から開講した状態と同じ形です。
乱数を使っていないので、何度シードしても同じ状態になります。
