# 共通基盤テンプレート（laravel-business-template）の解説

このドキュメントは、この予約システムのベースになっている**業務システム共通基盤**
（[laravel-business-template](https://github.com/koutadev/laravel-business-template)）の解説です。
認証・権限・共通マスタ・一覧基盤・監査ログ・デザインシステム・テーマなど、
**業務によらず使い回せる土台**の設計と使い方をまとめてあります。

予約システムそのもの（レッスン枠・予約・キャンセル待ちなど）については
