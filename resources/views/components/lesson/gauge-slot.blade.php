...
<a href="{{ route('lessons.show', $lesson->id) }}"
   title="{{ $lesson->starts_at->format('H:i') }} {{ $lesson->title }}／{{ $closed ? '受付終了' : $availability->description() }}"
   aria-label="{{ $lesson->starts_at->format('n月j日 H:i') }} {{ $lesson->title }} {{ $lesson->instructor?->name }} {{ $closed ? '受付終了' : $availability->description() }}"
   class="relative block flex-1 overflow-hidden rounded-lg border bg-white transition hover:ring-2 hover:ring-primary/40 motion-reduce:transition-none dark:bg-gray-800 {{ $closed ? 'border-gray-200 opacity-70 dark:border-gray-700' : $state->borderClass() }}">
    ...
    <span class="relative block px-2 py-1.5">
        ...
        <span class="mt-0.5 block text-[11px] tabular-nums {{ $closed ? 'text-gray-500 dark:text-gray-400' : $state->metaClass() }}">
            {{ $closed ? '受付終了' : $availability->gaugeLabel() }}
        </span>
    </span>
</a>
